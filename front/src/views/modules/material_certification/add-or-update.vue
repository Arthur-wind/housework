    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="申请原因" prop="application_reason">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.application_reason"
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.application_reason" label="申请原因" prop="application_reason">
                    <span v-html="ruleForm.application_reason"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shenqingziliao = true;
          ...
          if(o=='application_reason'){
            this.ruleForm.application_reason = obj[o];
	    this.ro.application_reason = true;
            continue;
          }
    ...
    info(id) {
      this.$http({
        url: `material_certification/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.application_reason = this.ruleForm.application_reason.replace(reg,'../../../springbootc90g5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `material_certification/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.material_certificationCrossAddOrUpdateFlag = false;
						 this.parent.search();
