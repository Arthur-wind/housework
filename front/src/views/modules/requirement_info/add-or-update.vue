<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="薪资" prop="salary">
          <el-input v-model="ruleForm.salary"
              placeholder="薪资" clearable  :readonly="ro.salary"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="薪资" prop="salary">
              <el-input v-model="ruleForm.salary"
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="工作时间" prop="working_hours">
          <el-input v-model="ruleForm.working_hours"
              placeholder="工作时间" clearable  :readonly="ro.working_hours"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="工作时间" prop="working_hours">
              <el-input v-model="ruleForm.working_hours"
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="工作地点" prop="work_location">
          <el-input v-model="ruleForm.work_location"
              placeholder="工作地点" clearable  :readonly="ro.work_location"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="工作地点" prop="work_location">
              <el-input v-model="ruleForm.work_location"
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="发布时间" prop="publish_time">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.publish_time"
                type="date"
                :readonly="ro.publish_time"
                placeholder="发布时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.publish_time" label="发布时间" prop="publish_time">
              <el-input v-model="ruleForm.publish_time"
                placeholder="发布时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.cover_image" label="封面" prop="cover_image">
          <file-upload
          tip="点击上传封面"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.cover_image?ruleForm.cover_image:''"
          @change="cover_imageUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.cover_image" label="封面" prop="cover_image">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.cover_image.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="要求" prop="requirements">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.requirements" label="要求" prop="requirements">
                    <span>{{ruleForm.requirements}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="详细说明" prop="detailed_description">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.detailed_description"
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.detailed_description" label="详细说明" prop="detailed_description">
                    <span v-html="ruleForm.detailed_description"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.publish_time = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.projectType = true;
          ...
          if(o=='salary'){
            this.ruleForm.salary = obj[o];
	    this.ro.salary = true;
            continue;
          }
          if(o=='working_hours'){
            this.ruleForm.working_hours = obj[o];
	    this.ro.working_hours = true;
            continue;
          }
          if(o=='work_location'){
            this.ruleForm.work_location = obj[o];
	    this.ro.work_location = true;
            continue;
          }
          if(o=='requirements'){
            this.ruleForm.requirements = obj[o];
	    this.ro.requirements = true;
            continue;
          }
          if(o=='detailed_description'){
            this.ruleForm.detailed_description = obj[o];
	    this.ro.detailed_description = true;
            continue;
          }
          if(o=='publish_time'){
            this.ruleForm.publish_time = obj[o];
	    this.ro.publish_time = true;
            continue;
          }
          if(o=='cover_image'){
            this.ruleForm.cover_image = obj[o];
	    this.ro.cover_image = true;
            continue;
          }
	    ...
	    this.ro.phone = true;
          ...
          if(o=='last_click_time'){
            this.ruleForm.last_click_time = obj[o];
	    this.ro.last_click_time = true;
            continue;
          }
          if(o=='click_count'){
            this.ruleForm.click_count = obj[o];
	    this.ro.click_count = true;
            continue;
          }
        }
      }
      ...
      this.$http({
		...
		if(json.phone!=''&&json.phone){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/service_project/project_name`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
	...
	this.ruleForm.detailed_description = this.ruleForm.detailed_description.replace(reg,'../../../springbootc90g5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `requirement_info/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.requirement_infoCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    cover_imageUploadChange(fileUrls) {
	this.ruleForm.cover_image = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
