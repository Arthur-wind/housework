<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.electronic_signature" label="电子签名" prop="electronic_signature">
          <file-upload
          tip="点击上传电子签名"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.electronic_signature?ruleForm.electronic_signature:''"
          @change="electronic_signatureUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.electronic_signature" label="电子签名" prop="electronic_signature">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.electronic_signature.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="签订日期" prop="sign_date">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.sign_date"
                type="date"
                :readonly="ro.sign_date"
                placeholder="签订日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.sign_date" label="签订日期" prop="sign_date">
              <el-input v-model="ruleForm.sign_date"
...
export default {
  ...
  created() {
	this.ruleForm.sign_date = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.content = true;
          ...
          if(o=='electronic_signature'){
            this.ruleForm.electronic_signature = obj[o];
	    this.ro.electronic_signature = true;
            continue;
          }
          if(o=='sign_date'){
            this.ruleForm.sign_date = obj[o];
	    this.ro.sign_date = true;
            continue;
          }
          if(o=='validity_period'){
            this.ruleForm.validity_period = obj[o];
	    this.ro.validity_period = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.employee_account!=''&&json.employee_account){
                this.ruleForm.employee_account = json.employee_account
	    		this.ro.employee_account = true;
		}
		if(json.employee_name!=''&&json.employee_name){
                this.ruleForm.employee_name = json.employee_name
	    		this.ro.employee_name = true;
		}
		if(json.contact_phone!=''&&json.contact_phone){
                this.ruleForm.contact_phone = json.contact_phone
	    		this.ro.contact_phone = true;
		}
        } else {
    ...
    info(id) {
      this.$http({
        url: `electronic_signature/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	if(this.ruleForm.electronic_signature!=null) {
		this.ruleForm.electronic_signature = this.ruleForm.electronic_signature.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `electronic_signature/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.electronic_signatureCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    electronic_signatureUploadChange(fileUrls) {
	this.ruleForm.electronic_signature = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
