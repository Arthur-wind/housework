<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="身份证" prop="id_card">
          <el-input v-model="ruleForm.id_card"
              placeholder="身份证" clearable  :readonly="ro.id_card"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="身份证" prop="id_card">
              <el-input v-model="ruleForm.id_card"
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.photo" label="照片" prop="photo">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.photo.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `employee/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.employeeCrossAddOrUpdateFlag = false;
						 this.parent.search();
