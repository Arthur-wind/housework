    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="预约备注" prop="appointment_notes">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.appointment_notes"
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.appointment_notes" label="预约备注" prop="appointment_notes">
                    <span v-html="ruleForm.appointment_notes"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.appointment_time = true;
          ...
          if(o=='appointment_notes'){
            this.ruleForm.appointment_notes = obj[o];
	    this.ro.appointment_notes = true;
            continue;
          }
    ...
    info(id) {
      this.$http({
        url: `service_appointment/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.appointment_notes = this.ruleForm.appointment_notes.replace(reg,'../../../springbootc90g5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `service_appointment/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.service_appointmentCrossAddOrUpdateFlag = false;
						 this.parent.search();
