<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.document" label="文件" prop="document">
          <file-upload
          tip="点击上传文件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.document?ruleForm.document:''"
          @change="documentUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.document" label="文件" prop="document">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.document)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.content = true;
          ...
          if(o=='document'){
            this.ruleForm.document = obj[o];
	    this.ro.document = true;
            continue;
          }
          if(o=='sign_date'){
	...
	if(this.ruleForm.document!=null) {
		this.ruleForm.document = this.ruleForm.document.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `contract_signing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.contract_signingCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    documentUploadChange(fileUrls) {
	this.ruleForm.document = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
