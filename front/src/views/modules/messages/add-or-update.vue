<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.content_picture" label="留言图片" prop="content_picture">
          <file-upload
          tip="点击上传留言图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.content_picture?ruleForm.content_picture:''"
          @change="content_pictureUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.content_picture" label="留言图片" prop="content_picture">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.content_picture.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.reply_picture" label="回复图片" prop="reply_picture">
          <file-upload
          tip="点击上传回复图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.reply_picture?ruleForm.reply_picture:''"
          @change="reply_pictureUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.reply_picture" label="回复图片" prop="reply_picture">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.reply_picture.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.content = true;
          ...
          if(o=='content_picture'){
            this.ruleForm.content_picture = obj[o];
	    this.ro.content_picture = true;
            continue;
          }
          if(o=='reply'){
	    ...
	    this.ro.reply = true;
          ...
          if(o=='reply_picture'){
            this.ruleForm.reply_picture = obj[o];
	    this.ro.reply_picture = true;
            continue;
          }
        }
      }
	...
	if(this.ruleForm.content_picture!=null) {
		this.ruleForm.content_picture = this.ruleForm.content_picture.replace(new RegExp(this.$base.url,"g"),"");
	}




	if(this.ruleForm.reply_picture!=null) {
		this.ruleForm.reply_picture = this.ruleForm.reply_picture.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    content_pictureUploadChange(fileUrls) {
	this.ruleForm.content_picture = fileUrls;
	this.addEditUploadStyleChange()
    },
    reply_pictureUploadChange(fileUrls) {
	this.ruleForm.reply_picture = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
